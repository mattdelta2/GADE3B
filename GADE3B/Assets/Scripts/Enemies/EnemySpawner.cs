...
{/*
    ...
    private void SpawnEnemy()
    {
        if (spawnPoints.Length > 0)
        {
            ...
            spawnPoint.y = terrain.SampleHeight(spawnPoint);  // Ensure spawn point is on the terrain

            // Log the spawn position to verify
            ...
            if (enemyController == null)
            {
                Debug.LogError("EnemyController not found on the instantiated enemy.");
            }
            if (enemyController != null)
            {
                // Generate a path from the spawn point to the main tower
                ...
                {
                    Debug.LogError("Generated path is null or empty.");
                }

                // Set the tower reference on the enemy
            ...
            // Move to the next spawn point, cycle back if at the end
