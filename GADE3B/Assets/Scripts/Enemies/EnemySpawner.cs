...
{/*
    ...
    private void Start()
    {
        // Start the process to wait for terrain and NavMesh to be ready before spawning
        StartCoroutine(WaitForNavMeshAndStartSpawning());
    }

    private IEnumerator WaitForNavMeshAndStartSpawning()
    {
        // Wait until the NavMesh is baked and the PathManager is ready
        ...
        else
        {
            ...
            yield break;  // Stop if PathManager is missing
        }
    ...
    private void SpawnEnemy()
    {
        if (spawnPoints.Length > 0)
        {
            ...
            {
                spawnPoint = hit.position;  // Use the NavMesh-adjusted position
